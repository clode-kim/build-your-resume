...
export default function ProjectSection({ data, onAdd, onUpdate, onDelete }: Props) {
  ...
  const handleTechKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, project: Project) => {
  ...
  const removeTech = (project: Project, tech: string) => {
    onUpdate(project.id, { techStack: project.techStack.filter((t) => t !== tech) });
  };
    ...
    <div className="space-y-4">
      {data.map((proj) => (
        <div key={proj.id} className="border border-slate-200 rounded-xl p-4 space-y-3 bg-white">
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {proj.techStack.map((tech) => (
                <span key={tech} className="inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-full">
                  {tech}
                  <button onClick={() => removeTech(proj, tech)} className="hover:text-indigo-900">×</button>
                </span>
              ))}
            </div>
            <input
          ...
          <button onClick={() => onDelete(proj.id)} className="text-xs text-red-400 hover:text-red-600 transition-colors">
            삭제
          </button>
        </div>
      ))}
